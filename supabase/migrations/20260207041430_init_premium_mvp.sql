...
-- 5) Job Assets (Structured file pointers)
...
CREATE TABLE IF NOT EXISTS public.jobs (
  ...
  input_audio_url text,                  -- optional link
...
-- Update assets backlink
ALTER TABLE public.job_assets ADD COLUMN IF NOT EXISTS job_id uuid REFERENCES public.jobs(id) on delete cascade;
...
VALUES ('assets', 'assets', false), ('outputs', 'outputs', false)
ON CONFLICT (id) DO NOTHING;

-- Storage Policies (Simplified for MVP)
CREATE POLICY "Assets: Users manage own assets" ON storage.objects
FOR ALL USING (
  bucket_id = 'assets' 
  AND (
...
CREATE POLICY "Outputs: Users manage own outputs" ON storage.objects
FOR ALL USING (bucket_id = 'outputs' AND auth.uid() = (storage.foldername(name))[1]::uuid);
